...
def extract_dataflow(code, parser, lang):
    ...
    try:
        ...
        tokens_index = tree_to_token_index(
            root_node)
        code = code.split('\n')
        code_tokens = [index_to_code_token(x, code) for x in
                       tokens_index]
        index_to_code = {}
        ...
        for id, e in enumerate(DFG):
            ...
            elif e[0] == "block" and DFG[id + 1][0] == "timestamp":
                critical_idx.append(DFG[id - 1][1])
                critical_idx.append(DFG[id + 2][1])
            elif e[0] == "token" and DFG[id + 1][0] == "transferFrom":
                critical_idx.append(DFG[id - 1][1])
                critical_idx.append(DFG[id + 2][1])
            elif e[0] == "delegatecall":
                critical_idx.append(DFG[id - 1][1])
                critical_idx.append(DFG[id + 2][1])
            elif e[0] == "deposit":
                critical_idx.append(DFG[id - 1][1])
